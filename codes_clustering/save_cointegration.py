...
class cointegration:
    ...
    def read_mom_data(self, data):
        # mom1 save and data Normalization
        mom1 = data[momentum_prefix_finder(data) + '1']
        data_normalized = pd.DataFrame((data - data.mean()) / data.std())

        # mom1을 제외한 mat/PCA(2-49)
        # df_combined = data_normalized.drop(columns=momentum_prefix_finder(data_normalized) + '1')

        # mom49를 제외한 mat/PCA(1-48)
        df_combined = data_normalized.drop(columns=momentum_prefix_finder(data_normalized) + '49')
